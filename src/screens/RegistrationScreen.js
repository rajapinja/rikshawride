...
function RegistrationScreen({ navigation }) {
  ...
  const validatePhone = (phone) => {
    ...
    const phonePattern = /^(?:\+?\d{10}|\+\d{2}\s?\d{10})$/;
  ...
  const handleRegistration = () => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      const registrationData = {
        ...
        phone: phone,
        role: selectedRole,
        vehicleType: selectedRole === 'AutoOwner' ? selectedVehicleType : null
      };
        axios.post(`${BASE_URL}/api/registration`, JSON.stringify(registrationData), {
      ...
      <ImageBackground source={backgroundImage} style={styles.background}>
        ...
        <View style={styles.container}>        
          <View style={styles.pageContent}>   
            <Text style={styles.title}>User Type Registration</Text>
            <View style={styles.transparentBox}>    
              <ScrollView contentContainerStyle={styles.scrollViewContent}>
                ...
                />     
                {selectedRole && selectedRole === 'AutoOwner' && (
                  <VehicleTypePicker
